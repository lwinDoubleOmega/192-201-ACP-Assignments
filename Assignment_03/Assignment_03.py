...
def calc(o): # o= orders
    global TAXRATE
    n = o[0]; t = o[1]; items = o[2] # n= customer name, t= tier, items= list of (product index, quantity)
    sub = 0.0; tax = 0.0
    print("Receipt for " + n + " (" + t + ")")
    print("-" * 40)
    for it in items:
        pi = it[0]; q = it[1] #product index, quantity
        p = PRODUCTS[pi][1]; nm = PRODUCTS[pi][0]; cat = PRODUCTS[pi][2]
        line = p * q # price * quantity
        sub = sub + line # subtotal + line
        if cat == "food": #cat = category
            tax = tax + line * foodtax
        else:
            tax = tax + line * TAXRATE
        print(nm + " x" + str(q) + " = " + str(line))
    d = 0.0
    # d = discount , t = tier, sub = subtotal
    if t == "none": 
        d = 0.0
    elif t == "silver":
...
# Customer class, tier subclasses, discount base on tiers and calculate_points start 
class Customer:          # base tier; then Silver / Gold / Platinum subclasses
    tier = "none"
    low_discount_rate = 0.0
    high_discount_rate = 0.0
    points_multiplier = 1
    ...
    def membership_discount(self, subtotal):
        if subtotal > DISCOUNT_THRESHOLD:
            rate = self.high_discount_rate
        else:
            rate = self.low_discount_rate
...
# Customer class, tier subclasses, discount base on tiers calculate_points end





# Order class, subtotal, total_quantity, discount, tax, total, points, receipt start
class Order:              # has-a Customer, has-many OrderItem
    ...
    def discount(self):     # pure
        ...
        discount = self.customer.membership_discount(subtotal)
    ...
    def tax(self):          # pure
        ...
        for item in self.items:
            if item.product.category == FOOD_CATEGORY:
                tax += item.line_total() * FOOD_TAX_RATE
            else:
                tax += item.line_total() * TAX_RATE
        return tax
    ...
    def points(self):
        return self.customer.calculate_points(self.total())

        
    def receipt(self):          # builds the receipt text (no calculation here)
        print(f"Receipt for {self.customer.name} ({self.customer.tier})")
        print("-" * 40)
        for item in self.items:
        ...
        print("") 
#order class, subtotal, total_quantity, discount, tax, total, points, receipt end
